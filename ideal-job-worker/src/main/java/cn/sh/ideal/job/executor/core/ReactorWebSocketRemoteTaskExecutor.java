 ...
 * @date 2020/8/20
 */
public final class ReactorWebSocketRemoteTaskExecutor extends Thread implements RemoteTaskExecutor {
  private static final Logger log = LoggerFactory.getLogger(ReactorWebSocketRemoteTaskExecutor.class);
...
//    Runtime.getRuntime().addShutdownHook(new Thread(this::destroy));
  ...
  public void execute(@Nonnull TaskParam param) {
    JobExecutor.getExecutor().executeJob(param);
  }
  ...
  public void taskCallback(@Nonnull TaskCallback callback) {
    ...
    SocketMessage socketMessage = new SocketMessage(TaskCallback.typeCode, callbackMessage);
