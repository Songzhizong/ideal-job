...
import com.zzsong.job.scheduler.core.admin.service.InstanceLogService;
...
import com.zzsong.job.scheduler.core.converter.InstanceLogConverter;
 ...
 */
@Controller
public class RSocketExecutorServer {
  private static final Logger log = LoggerFactory.getLogger(RSocketExecutorServer.class);
  @Nonnull
  ...
  public RSocketExecutorServer(@Nonnull Scheduler blockScheduler,
                               @Nonnull LbFactory<TaskExecutor> lbFactory,
                               @Nonnull JobInstanceService instanceService,
                               @Nonnull InstanceLogService instanceLogService,
   ...
   * 客户端与服务端建立连接
   */
  @ConnectMapping(ExecutorRouter.LOGIN)
  void login(@Nonnull RSocketRequester requester, @Payload String loginMessage) {
    ...
    RSocketTaskExecutor[] warp = new RSocketTaskExecutor[1];
    requester.rsocket()
        .onClose()
        .doFirst(() -> {
          if (StringUtils.isNotBlank(propertiesAccessToken)
            ...
            RSocketTaskExecutor executor
                = new RSocketTaskExecutor(appName, instanceId, requester);
            warp[0] = executor;
            executor.setWeight(weight);
            lbFactory.addServers(appName, ImmutableList.of(executor));
          }
        })
        .doOnError(error -> {
        ...
        .doFinally(consumer -> {
          RSocketTaskExecutor executor = warp[0];
          if (executor != null) {
            lbFactory.markServerDown(appName, executor);
          }
...
//    }
   ...
   * task执行完成回调
   */
  @MessageMapping(ExecutorRouter.TASK_CALLBACK)
   ...
   * 接收任务执行过程中产生的日志
   */
  @MessageMapping(ExecutorRouter.INSTANCE_LOG)
  Mono<Void> instanceLogReport(@Nonnull InstanceLogReport report) {
    InstanceLog instanceLog = InstanceLogConverter.fromInstanceLogReport(report);
    blockScheduler.schedule(() -> instanceLogService.save(instanceLog).subscribe());
    return Mono.empty();
  }
}
