package com.zzsong.job.executor;
...
import com.zzsong.job.executor.handler.IJobHandler;
import com.zzsong.job.executor.handler.JobHandlerFactory;
import com.zzsong.job.executor.socket.ProtocolTypeEnum;
import com.zzsong.job.executor.socket.ReactorWebSocketRemoteTaskExecutor;
import com.google.common.util.concurrent.ThreadFactoryBuilder;
import com.zzsong.job.executor.socket.rsocket.RSocketRemoteTaskExecutor;
...
public class JobExecutor {
  ...
  private static final LbFactory<RemoteTaskExecutor> lbFactory = new SimpleLbFactory<>();
  ...
  private final List<RemoteTaskExecutor> remoteTaskExecutors = new ArrayList<>();
  ...
  public void destroy() {
    if (!destroyed) {
      ...
      destroyed = true;
      for (RemoteTaskExecutor remoteExecutor : remoteTaskExecutors) {
  ...
  private void initRemoteWorks() {
    ...
    lbFactory.addServers(SCHEDULER_SERVER_NAME, remoteTaskExecutors);
  }
  ...
  private void initRSocketRemoteWorks() {
    ...
    for (String address : addresses) {
      ...
      RSocketRemoteTaskExecutor executor = new RSocketRemoteTaskExecutor(ip, port);
      executor.setAppName(appName);
      executor.setWeight(weight);
      executor.setAccessToken(accessToken);
      executor.setExecutorIp(executorIp);
      executor.setExecutorPort(executorPort);
      executor.startExecutor();
      remoteTaskExecutors.add(executor);
    }
  }
  ...
  private void initWebsocketRemoteWorks() {
    ...
    for (String address : addresses) {
      ...
      executor.setWeight(weight);
      executor.setAccessToken(accessToken);
      executor.setConnectTimeOutMills(CONNECT_TIME_OUT_MILLS);
      executor.setWriteTimeOutMills(WRITE_TIME_OUT_MILLS);
      executor.setReadTimeOutMills(READ_TIME_OUT_MILLS);
      executor.start();
      remoteTaskExecutors.add(executor);
    }
  }
  ...
  @Nullable
  public RemoteTaskExecutor chooseRemoteJobExecutor() {
    return lbFactory.chooseServer(SCHEDULER_SERVER_NAME, null);
  }

  @Nullable
  public RemoteTaskExecutor chooseRemoteJobExecutor(int retry) {
    if (retry < 1) {
      return chooseRemoteJobExecutor();
    }
    RemoteTaskExecutor remoteJobExecutor = null;
    int i = -1;
    ...
    try {
      executorService.execute(() -> {
        ...
        try {
          ...
          RemoteTaskExecutor executor = chooseRemoteJobExecutor(2);
          // 任务完成时间尽量返回服务端
          if (executor != null) {
